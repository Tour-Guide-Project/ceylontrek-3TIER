<?php
    ...
    function make_reservation_query($connection,$tourist_id,$guide_id,$tourist_name,$tourist_email,$telephone_number,$no_of_adults,$no_of_children,$arrival_date,$departure_date,$notes,$total_price){
        ...
        $query="INSERT INTO reservations(tourist_id, guide_id, tourist_name,tourist_email,telephone, arrival_date, departure_date, no_of_adults, no_of_children, notes,price) 
        VALUES ('{$tourist_id}','{$guide_id}','{$tourist_name}','{$tourist_email}','{$telephone_number}','{$arrival_date}','{$departure_date}','{$no_of_adults}','{$no_of_children}','{$notes}','{$total_price}')";
        $result= mysqli_query($connection,$query);
        return $result;} 
